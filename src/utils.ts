...
export const getCurrentWeather = (locationDetails: Location) : Promise<WeatherResponse> => {
...
export const displayLocation = (locationDetails: Location) : void => {
    const locNameElm = document.getElementById('location-name') as HTMLElement
    const countryElm = document.getElementById('country') as HTMLElement

    locNameElm.innerText = locationDetails.name
    countryElm.innerText = "(" + locationDetails.country + ")";
}

export const displayWeatherDate = (obj: WeatherResponse) : void => {
...
export const updatedBackground = (weatherCode: number, isDay: number) : void => {
